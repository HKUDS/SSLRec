...
def build_sim_graph(user_history_lists, mode):
    data_path = path.join("./datasets", configs['data']['name'])
    k = configs['model']['sim_group_k']
    graph_file = path.join(data_path, f"/sim_graph_{k}_{mode}.bin")
    try:
        ...
        cf_graph = sp.csr_matrix(([1]*len(row), (row, col)), shape=(
            configs['data']['item_num']+1, configs['data']['item_num']+1), dtype=np.float32)
...
def build_adj_graph(user_history_lists, mode):
    data_path = path.join("./datasets", configs['data']['name'])
    graph_file = path.join(data_path, f"/adj_graph_{mode}.bin")
    user_edges_file = path.join(data_path, "/user_edges.pkl.zip")
    try:
        g = dgl.load_graphs(graph_file, [0])
...
class SequentialDataset(data.Dataset):
    def __init__(self, user_seqs, mode='train'):
        ...
        self.user_history_lists = {user: seq for user,
                                   seq in zip(self.uids, user_seqs["item_seq"])}
